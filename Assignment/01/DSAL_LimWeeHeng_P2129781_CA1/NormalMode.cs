    ...
    public partial class NormalMode : Form
    {
        ...
        private void createSeat(object sender, EventArgs e)
        {
            try
            {
                ...
                /*
                int colDivide = int.Parse(textBoxColumnDivider.Text);
                int rowDivide = int.Parse(textBoxRowDivider.Text);
                */
                ...
                for (int x = 1; x <= numRow; x++) // creates Rows
                {
                    for (int y = 1; y <= numColumns; y++)// creates Columns
                    {
                        ...
                        if(y == numColumns)// check to see if its the last label of column
                        {
                            seat.isLastSeat = true;
                        }
                        if(y == 1) // check to see if its the first label of column
                        {
                            seat.isFirstSeat = true;
                        }
                        if(x == 1 && y == 1) // check to see if its the first label of list
                        {
                            seat.isStartOfList = true;
                        }

                        if(x == numRow && y == numColumns) // check to see if its the last label of list
                        {
                            seat.isEndOfList = true;
                        }

                        /*
                        if(y == colDivide) // check for column divider
                        {
                            seat.hasDivider = true;
                            labelSeat.Location = new Point((80 * (y - 1)) + 50 + (20 * (y - 1)), (60 * (x - 1)) + 50 + (20 * (x - 1)));
                        }

                        if(x == rowDivide) // check for row divider
                        {
                            labelSeat.Location = new Point((60 * (y - 1)) + 50 + (20 * (y - 1)), (80 * (x - 1)) + 50 + (20 * (x - 1)));
                        }
                        */
